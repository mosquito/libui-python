...
static int window_closing_trampoline(uiWindow *sender, void *data);
...
Window_init(PyObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (rc == 0) {
        ...
        /* Install our trampoline as the default on_closing handler.
         * libui-ng's built-in default returns 0 (deny close), which makes
         * the close button appear to hang.  Our trampoline defaults to
         * allowing close + quitting when no Python callback is registered. */
        uiWindowOnClosing(w, window_closing_trampoline, (void *)as_ctrl(self));
    }
    return rc;
}
...
window_closing_trampoline(uiWindow *sender, void *data)
{
    ...
    if (callable == NULL) {
        if (PyErr_Occurred())
            ...
            /* No callback registered — default: allow close and quit.
             * Call fire_should_quit() instead of uiQuit() to directly
             * invoke the Python on_should_quit callback (which sets
             * quit_event).  Calling uiQuit() from within windowShouldClose:
             * goes through [NSApp terminate:] → applicationShouldTerminate:
             * which has Cocoa reentrancy issues on macOS. */
            ret = 1;
            fire_should_quit();
        }
    } else {
    ...
    if (ret) {
        ...
        /* Ensure the Python event loop is signalled to quit.
         * fire_should_quit() calls _on_should_quit → quit_event.set().
         * This is safe to call even if the user's callback already called
         * quit() — quit_event.set() is idempotent. */
        fire_should_quit();
    }
