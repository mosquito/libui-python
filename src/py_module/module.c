...
mod_uninit(PyObject *Py_UNUSED(self), PyObject *Py_UNUSED(args))
{
    ENSURE_MAIN_THREAD;
    uninit_all_controls();
    /* Run GC to free unreferenced Python objects that hold libui resources */
    PyGC_Collect();
    /* Free any remaining non-Control libui resources (Image, AttributedString,
     * DrawPath, etc.) that are still alive as Python objects */
...
static PyObject *
mod_is_main_thread(PyObject *Py_UNUSED(self), PyObject *Py_UNUSED(args))
{
    return PyBool_FromLong(is_main_thread());
}
...
void
fire_should_quit(void)
{
    if (should_quit_callback != NULL) {
        PyObject *result = PyObject_CallNoArgs(should_quit_callback);
        if (result == NULL)
            PyErr_WriteUnraisable(should_quit_callback);
        else
            Py_DECREF(result);
    }
}
...
static PyMethodDef module_methods[] = {
    ...
    {"is_main_thread",          mod_is_main_thread,                              METH_NOARGS,                  "is_main_thread()\n--\n\nReturn True if the current thread is the main UI thread."},
