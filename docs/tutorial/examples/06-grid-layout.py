...
async def main():
    ...
    keypad = Grid(
        ...
        # Row 1
        btn("7", 0, 1), btn("8", 1, 1), btn("9", 2, 1),
        GridCell(Button("C", on_clicked=clear), 3, 1),
        # Row 2
        btn("4", 0, 2), btn("5", 1, 2), btn("6", 2, 2),
        btn("+", 3, 2),
        # Row 3
        btn("1", 0, 3), btn("2", 1, 3), btn("3", 2, 3),
        btn("-", 3, 3),
        # Row 4: zero spans 2 columns
        btn("0", 0, 4, xspan=2), btn(".", 2, 4),
        btn("=", 3, 4),
        padded=True,
    )

    app.build(window=Window(
        "Grid Layout — Calculator", 300, 250,
        child=VBox(keypad),
    ))
