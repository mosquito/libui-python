...
async def main():
    ...
    menus = [
        MenuDef("File",
            MenuItem("Open...", on_click=lambda: status.set(
                f"Open: {app.open_file() or '(cancelled)'}"
            )),
            MenuItem("Save As...", on_click=lambda: status.set(
        ...
        MenuDef("Edit",
            CheckMenuItem("Dark Mode", checked=dark_mode,
                          on_click=lambda: status.set(
                              f"Dark mode: {dark_mode.value}"
                          )),
            MenuSeparator(),
            PreferencesItem(),
        ),
        MenuDef("Help",
            AboutItem(),
            MenuItem("Documentation", on_click=lambda: app.msg_box(
    ...
    app.build(
        menus=menus,
        window=Window("Menus", 500, 250, has_menubar=True, child=VBox(
            Label(text=status),
        )),
    )
