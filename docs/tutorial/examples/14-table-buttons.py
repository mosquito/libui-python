...
from libui.declarative import (
    App, Window, VBox,
    Label, State, stretchy,
    DataTable, ListState, TextColumn, ProgressColumn, ButtonColumn,
)
    ...
    app.build(window=Window(
        ...
        child=VBox(
            ...
            stretchy(DataTable(
                ...
                on_row_clicked=lambda row: status.set(f"Selected: {data[row]['name']}"),
            )),
        ),
    ))
